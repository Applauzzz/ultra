...
class OptimArgs:
    ...
    cycle_length: float = 1.0 # single turn of lr scheduler
...
def lr_wsd(
    ...
    """
    cycle_num = step // int(n_steps * cycle_length) + 1 # current cycle number
    curr_n_steps = int(n_steps * cycle_length) * cycle_num # step of the end of current cycle
    decay_length = int(curr_n_steps * decay_fraction) # length of decay for current cycle
    if step == n_steps:
        cycle_num -= 1
    ...
    if step < warmup:
        ...
        if mode == "linear":
            step_left_in_decay = decay_length - (curr_n_steps - step)
            lr = min_ratio + (1.0 - min_ratio) * step_left_in_decay / decay_length
        elif mode == "inv":
