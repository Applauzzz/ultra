...
def loop_on_hf_dataset(
    ...
    if load_type == "load_dataset":
        if file_path == "HuggingFaceFW/fineweb-edu/sample-350BT":
            ...
            if len(parts) == 2:
                ...
                path = file_path
                name = None
                print(f"Detected Dataset: path='{path}'")
                dataset = load_dataset(file_path, name='default', split="train")
            elif len(parts) >= 3:
                # 情况 2: "google/fleurs/en_us"
                # 假设前两部分是 path，第三部分是 name (subset)
                path = "/".join(parts[:2])
                name = "/".join(parts[2:]) # 处理 name 中可能包含斜杠的极端情况，虽然很少见
                print(f"Detected Dataset with Subset: path='{path}', name='{name}'")
                dataset = load_dataset(path, name=name, split="train")
    ...
    try:
        while True:
            it = read_hf_dataset(ds, file_path, position, block_size, offset, current_iter, load_type)
            for content, jsonl_state in it:
                yield content, jsonl_state
            current_iter += 1
            position = 0
