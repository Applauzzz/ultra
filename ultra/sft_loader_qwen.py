...
class SFTDataArgs:
    ...
    pad_token_id: int = 151643  # qwen3  pad_token_id !!!!!!!!!!!!!!
...
def example_usage():
    model_path = "/mnt/jfzn/lzh/modelbase/Qwen3-8B"

    sft_args = SFTDataArgs(
        sources={"load_from_disk./mnt/jfzn/lzh/database/tulu-3-sft-mixture_hfds": 1.0},
        batch_size=8,
        max_seq_len=2048,
    ...
    with build_sft_dataloader(state) as dataloader:
        for batch, new_state in dataloader:
            ...
            state = new_state
            tokenizer = build_tokenizer(name="huggingface", path=model_path)
            print("===== input_ids[0] =====")
