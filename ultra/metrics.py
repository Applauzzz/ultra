...
class MetricLogger:
    ...
    def log(self, metrics: Dict[str, Any]):
        if (
            ...
            wandb.log(metrics, step=metrics["global_step"], commit=True)

            # force flush
            try:
                wandb.run._backend.interface.publish_history()
            except Exception:
                pass
...
class GPUMemoryMonitor:
    ...
    def get_peak_stats(self):
        ...
        try:
            power_draw = torch.cuda.power_draw(dev)
        except Exception:
            power_draw = None  # 监控失败不影响训练
        if num_retries > 0:
