...
def validate_train_args(args: TrainArgs, output_size: int):
    ...
    assert (
        ...
        * args.distributed.sp_size
        == get_world_size()
    ), "DP * TP * SP should be equal to world size but got {} * {} * {} * {} != {}".format(
        args.distributed.dp_replicate,
        args.distributed.dp_shard,
        args.distributed.tp_size,
        args.distributed.sp_size,
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        while train_state.step < args.steps:
            ...
            if train_state.step == 0 and train_state.acc_step == 1:
                print("input_ids.shape =", input_ids.shape)
                print("seq_len config =", args.data.seq_len)
                print("model max_seqlen =", args.model.max_seqlen)
            if args.varlen:
                output = model(input_ids=input_ids, labels=labels, cu_seqlens=cu_seqlens)
            else:
                output= model(input_ids=input_ids, labels=labels)
            ...
            # if model.memory_efficient_checkpointing == False:
            loss.backward()
            # loss.backward()
