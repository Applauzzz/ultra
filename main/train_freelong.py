...
class TrainState(Stateful):
    ...
    data_loader_state: Optional[PackTokensState] = None
...
def validate_train_args(args: TrainArgs, output_size: int):
    ...
    # assert (
    #     args.model.vocab_size == output_size
    # ), f"Vocab size {args.model.vocab_size} should be the same as output size {output_size}"
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        if sp_degree > 1:
            ...
            # sp_list = world_mesh["sp"].mesh.tolist()
            # if (dist.get_world_size() == 8) and (dist.get_rank() == 4):
            #     import pdb
            #     pdb.set_trace()
            # dist.barrier()
            # sp_group = dist.new_group(sp_list, timeout=timedelta(minutes=150), backend="nccl")
        ...
        while train_state.step < args.steps:
            ...
            if args.random_data:
                # vocab size：优先用 tokenizer.n_words，其次用 args.model.vocab_size
                print("Using random data for training ......")
                vocab_size = tokenizer.n_words if tokenizer is not None else args.model.vocab_size
                assert vocab_size and vocab_size > 0, f"Invalid vocab_size={vocab_size}"

                if sp_degree > 1:
                    # ✅ 每个 SP rank 只生成本 rank 的片段：seq_len / sp_degree
                    local_seqlen = args.data.seq_len // sp_degree
                    # 形状保持成 [bsz=1, local_seqlen]
                    ...
                    input_ids = torch.randint(
                        low=0,
                        high=vocab_size,
                        size=(1, args.data.seq_len),
                        device="cuda",
                        dtype=torch.long,
                    )

                # 可选：把最后一个 token 设为 eos，方便 varlen/或一些loss边界行为更稳定
                if tokenizer is not None and hasattr(tokenizer, "eos_id") and tokenizer.eos_id is not None:
                    input_ids[:, -1] = tokenizer.eos_id

                labels = input_ids.clone()
                cu_seqlens = None
                if args.varlen:
                    # ✅ varlen 下给一个最简单的 “单条序列” cu_seqlens
                    # 注意：不同实现可能期望 int32/cuda
                    # 这里长度就是当前 rank 的 local_seqlen（SP>1）或全长
                    seqlen_here = input_ids.shape[1]
                    cu_seqlens = torch.tensor([0, seqlen_here], device="cuda", dtype=torch.int32)

            else:
                # ---- 原来的真实数据路径 ----
                if sp_degree > 1:
                    batch, train_state.data_loader_state, cu_seqlens = sp_dataloader(
                        train_state.step, sp_group, sp_rank, sp_degree,
                        train_state.data_loader_state, args.data, data_loader,
                        args.varlen, eos_id
                    )
                    input_ids = batch[:, :, 0]
                    labels = input_ids.clone()
                    ...
                    batch, train_state.data_loader_state = next(data_loader)
                    batch = torch.tensor(batch, dtype=torch.long, device="cuda")  # [bsz, seqlen, 1]
                    input_ids = batch[:, :, 0]
                    labels = input_ids.clone()
                    cu_seqlens = None
                    if args.varlen:
            ...
            # model.disable_memory_efficient_checkpointing()
            _chunk_size = args.model.chunk_size
            if get_is_master():
                logger.info(f"[chunk-ab] FreeLong chunk_size = {_chunk_size}")
            model.enable_memory_efficient_checkpointing(chunk_size=_chunk_size)
            # model.enable_memory_efficient_checkpointing(chunk_size=32*1024)
            # model.enable_tbptt_training(chunk_size=4*1024)
            if args.varlen:
                output,_ = model(input_ids=input_ids, labels=labels, cu_seqlens=cu_seqlens)
            else:
                output,_ = model(input_ids=input_ids, labels=labels)
            loss = output.loss
            ...
            if model.memory_efficient_checkpointing == False and model.tbptt_training == False:
                loss.backward()
            # loss.backward()
            ...
            if every_n_steps(
                ...
                tgs = nwords_since_last_log / (time_delta * args.distributed.tp_size)
