...
os.environ["TRITON_ENABLE_AUTOTUNING"] = "0"
...
from ultra.sft_loader import (
    init_sft_dataloader_state,
    build_sft_dataloader_from_args,
)
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        if args.checkpoint.init_ckpt_path and args.model.meta_init:
            ...
            # print("Expect keys sample:", list(state_dict.keys())[:20])
        ...
        # )
        data_loader_state = init_sft_dataloader_state(
            args.data, loader_rank, loader_degree
        )
        ...
        while train_state.step < args.steps:
            ...
            # get batch
            if sp_degree > 1:
                UserWarning("SP is not supported for SFT training, use DP instead")
                raise Exception("SP is not supported for SFT training, use DP instead")
                ...
                if dist.get_rank() == 0 and train_state.step == 0 and train_state.acc_step == 0:
                    from pprint import pprint

                    tokenizer_dbg = tokenizer  # 你前面 build_tokenizer 得到的

                    # 只看 batch 的第一条
                    ids = input_ids[0].detach().cpu().tolist()
                    lbl = labels[0].detach().cpu().tolist()

                    # 哪些 token 参与 loss
                    valid_mask = [l != -100 for l in lbl]

                    print("\n" + "=" * 80)
                    print("[DEBUG] First sample raw ids")
                    print(ids[:200])
                    print("\n[DEBUG] First sample raw labels")
                    print(lbl[:200])
                    print("\n[DEBUG] First sample valid label mask (1=loss, 0=ignore)")
                    print([1 if x else 0 for x in valid_mask[:200]])

                    # decode（保留 special tokens）
                    text_input = tokenizer_dbg.tokenizer.decode(
                        ids,
                        skip_special_tokens=False,
                    )
                    print("\n[DEBUG] First sample decoded input ids")
                    print(text_input)
                    # 统计信息
                    valid_cnt = sum(valid_mask)
                    total_cnt = len(lbl)
                    print("\n" + "-" * 80)
                    print(f"[DEBUG] valid label ratio: {valid_cnt}/{total_cnt} = {valid_cnt/total_cnt:.4f}")
                    print("=" * 80 + "\n")
                ...
                cu_seqlens = None
                if args.varlen:
                    UserWarning("Varlen is not supported for SFT training, use fixed seqlen instead")
                    raise Exception("Varlen is not supported for SFT training, use fixed seqlen instead")
